//#region dsh-start&exit 浏览器客户端 bundle
/**
 * dsh-start&exit 客户端：
 * - 在 dsh 设置菜单中注册一个始终排在最后的“启动与退出”栏目
 * - 提供对齐展示的启动、退出与重新启动操作；确认后分别调用
 *   remote.dshStartExit.start() / remote.dshStartExit.exit() /
 *   remote.dshStartExit.restart()
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const START_BLUE = "var(--dsw-alias-state-business-primary)";
		const RESTART_BLUE = START_BLUE;
		...
		const START_ICON_SVG = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3v12"/><path d="m7 10 5 5 5-5"/><path d="M5 21h14"/></svg>';
		...
		const CSS_TEXT = [
			...
			".dshExitActionIcon--start{color:" + START_BLUE + "}",
			...
			".dshExitSettingsButton--start{background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground)}",
			...
			".dshExitSettingsButton--start:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}",
			...
			".dshExitConfirm--start{background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground)}",
			...
			".dshExitConfirm--start:hover:not(:disabled){background:var(--dsw-alias-button-primary-hover)}",
		...
		/** 客户端 Remote 描述符：挂载后 ctx.get("remote.dshStartExit") 可用。 */
		const START_EXIT_REMOTE = {
		...
		const NS = "dsh-start-and-exit";
		const DICT = {
			zh: {
				nav: "启动与退出",
				title: "启动、退出与重新启动 dsh",
				description: "从这里通过可见的 Windows Terminal 安全启动、结束或重新启动 dsh 宿主进程。",
				startActionTitle: "启动 dsh",
				startWarning: "从 Windows Terminal 的 PowerShell 7 启动 dsh web profile；当前服务会先释放端口。桌面 Chrome 应用快捷方式仍需单独配置启动器。",
				startButton: "启动 dsh",
				...
				startDialogTitle: "启动 dsh？",
				startDialogBody: "这会先结束当前运行中的会话，再在可见的 Windows Terminal（PowerShell 7）中启动 dsh web profile。服务恢复后页面将自动刷新。",
				cancel: "取消",
				confirm: "确认退出",
				restartConfirm: "确认重启",
				exiting: "正在退出…",
				restarting: "正在重新启动…",
				starting: "正在启动 dsh…",
				startWaiting: "dsh 正在新的可见 Windows Terminal 中启动，恢复后页面将自动刷新。",
				restartWaiting: "后台服务正在重新启动，恢复后页面将自动刷新。",
				restartTimeout: "后台服务尚未恢复，请稍后手动刷新页面。",
				exitErrorPrefix: "退出失败：",
				restartErrorPrefix: "重启失败：",
				startErrorPrefix: "启动失败："
			},
			en: {
				nav: "Start & Exit",
				title: "Start, exit, or restart dsh",
				description: "Safely start, terminate, or restart the dsh host in a visible Windows Terminal.",
				startActionTitle: "Start dsh",
				startWarning: "Starts the dsh web profile in PowerShell 7; the current service releases its port first. The desktop Chrome app shortcut still needs a separate launcher configuration.",
				startButton: "Start dsh",
				...
				startDialogTitle: "Start dsh?",
				startDialogBody: "This ends the current sessions, then starts the dsh web profile in a visible Windows Terminal (PowerShell 7). The page refreshes after recovery.",
				cancel: "Cancel",
				...
				startWaiting: "dsh is starting in a new visible Windows Terminal. This page will refresh after it recovers.",
		...
		function decorateExitNav(t) {
			const labels = new Set(["退出", "Exit", "启动与退出", "Start & Exit", textOf(t, "nav")]);
		...
		function waitForService({ title, body, errorEl, actions, cancel, confirm, t, pendingKey, waitingKey }) {
			const startedAt = Date.now();
			let sawUnavailable = false;
			title.textContent = textOf(t, pendingKey);
			body.textContent = textOf(t, waitingKey);
		...
		function openModal(ctx, actionBtn, t, action) {
			...
			const isLaunch = isStart || isRestart;
			const keys = isStart ? {
			...
			confirm.type = "button";
			confirm.className = "dshExitConfirm" + (isLaunch ? " dshExitConfirm--" + action : "");
			...
			confirm.addEventListener("click", () => {
				...
				Promise.resolve()
					.then(() => {
						const service = ctx.get("remote.dshStartExit");
						if (service === undefined) throw new Error("dsh-start&exit 服务不可用（宿主未注册？）");
						if (typeof service[action] !== "function") throw new Error("dsh-start&exit 不支持 " + action + " 操作");
		...
		async function apply(ctx) {
			injectCss();
			ctx.effect(() => ctx.locale.register(NS, DICT), "dsh-start&exit: settings dictionaries");
			...
			if (remote !== undefined) disposeRemote = await remote.$mount(START_EXIT_REMOTE);
			return async () => {
